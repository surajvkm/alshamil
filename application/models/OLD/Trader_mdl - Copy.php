...
class Trader_mdl extends CI_Model {
    ...
    function get_post_byid($post_id) {
        $qry = $this->db->query('SELECT DISTINCT `post`.`postID`,post.productCategoryID,post.productID,productiv.productViewCount ,
           `productcar`.`productCBrand`,DATE_FORMAT(productcar.productCSubmitDate,"%d %b %Y") as publishedCOn ,`productcar`.`productCCallPrice`, `productcar`.`productCModel`,productcar.productCReleaseYear, `productcar`.`productCPrice`,productcar.productCStatus,productcar.productCDesc,productcar.Cpost_main_img,
           `productbike`.`productBBrand`,`productbike`.`productBCallPrice`, `productbike`.`productBModel`, DATE_FORMAT(productbike.productBSubmitDate,"%d %b %Y") as publishedBOn,productbike.productBReleaseYear as Bikeyear, `productbike`.`productBPrice`,productbike.productBStatus,productbike.Bpost_main_img,productbike.productBDesc as bike,
           `productboat`.`productBtBrand`,DATE_FORMAT(productboat.productBTSubmitDate,"%d %b %Y") as publishedBtOn,`productboat`.`productBtCallPrice`, `productboat`.`productBtModel`, `productboat`.`productBTSubmitDate`,productboat.productBReleaseYear, `productboat`.`productBTPrice`,productboat.productBTStatus,productboat.productBDesc,productboat.BTpost_main_img, 
           `productwatch`.`productWBrand`,DATE_FORMAT(productwatch.productWSubmitDate,"%d %b %Y") as publishedWOn,`productwatch`.`productWCallPrice`, `productwatch`.`productWModel`, `productwatch`.`productWSubmitDate`, `productwatch`.`productWPrice`,productwatch.productWStatus,productwatch.productWDesc,productwatch.Wpost_main_img,
           `productvertu`.`productVBrand`,DATE_FORMAT(productvertu.productVSubmitDate,"%d %b %Y") as publishedVOn,`productvertu`.`productVCallPrice`, `productvertu`.`productVModel`, `productvertu`.`productVSubmitDate`, `productvertu`.`productVPrice`,productvertu.productVStatus,productvertu.productVDesc,productvertu.Vpost_main_img, 
           `productproperty`.`productPropSC`,DATE_FORMAT(productproperty.productPRSubmitDate,"%d %b %Y") as publishedPROn ,`productproperty`.`productPropCallPrice`,`productproperty`.`productPropType`,  `productproperty`.`productPRSubmitDate`, `productproperty`.`productPRPrice`,productproperty.productPRStatus,productproperty.productDesc,productproperty.PRpost_main_img, 
           `productphone`.`productPBrand`,DATE_FORMAT(productphone.productPSubmitDate,"%d %b %Y") as publishedPOn,`productphone`.`productPhCallPrice`, `productphone`.`productPModel`, `productphone`.`productPSubmitDate`, `productphone`.`productPHPrice`,productphone.productPHStatus,productphone.productPDesc,productphone.PHpost_main_img,
           `productnp`.`productNPCode`,DATE_FORMAT(productnp.productNPSubmitDate,"%d %b %Y") as publishedNPOn,`productnp`.`productNPCallPrice`, `productnp`.`productNPDigits`, `productnp`.`productNPSubmitDate`, `productnp`.`productNPPrice`,productnp.productNPStatus,productnp.productNPDesc,productnp.NPpost_main_img,productnp.productNPNmbr,productnp.productNPEmrites,
           `productmn`.`productMNPrefix`,DATE_FORMAT(productmn.productMNSubmitDate,"%d %b %Y") as publishedMNOn,`productmn`.`productMNCallPrice`, `productmn`.`productMNNmbr`, `productmn`.`productMNSubmitDate`, `productmn`.`productMNPrice`,productmn.productMNStatus,productmn.productMNDesc,productmn.MNpost_main_img,productmn.productOperator
...
//           
    ...
    function get_post_lists($per_page_cnt, $limit) {
      ...
      productmn.cartMNType, productmn.productMNSubmitDate,productmn.productMNPrice,productmn.MNpost_main_img,productmn.productOperator,productmn.productMNNmbr,productmn.productMNDesc,productmn.productMNCallPrice,productmn.productMNStatus,productmn.productOperator,trader.traderID FROM `post`
        ...
        left join productproperty on (post.productCategoryID=productproperty.productCategoryID and post.productID=productproperty.productID)
        left join productvertu on (post.productCategoryID=productvertu.productCategoryID and  post.productID=productvertu.productID)
        left join productwatch on (post.productCategoryID=productwatch.productCategoryID and post.productID=productwatch.productID)  order by post.postSubmissionOn DESC  LIMIT '.$limit.','.$per_page_cnt);
    ...
    function filter($categoryId, $carBrand, $carModel, $carStartDate, $carEndDate, $bikeBrand, $bikeModel, $bikeStartDate, $bikeEndDate, $npEmirate, $npCode, $npDigitCount, $vertuBrand, $vertuModel, $watchBrand, $watchModel, $mnPrefix, $mnOperator, $mnDigitCount, $boatBrand, $boatModel, $phoneBrand, $phoneModel, $propertiesCategory, $propertiesSubCategory, $per_page_cnt, $limit) {
        ...
        if ($categoryId == 1) {
          ...
          ,DATE_FORMAT(productbike.productBSubmitDate,'%d %b %Y') as publishedOn,`productbike`.`productBCallPrice` as callprice
           ...
           left JOIN `trader` ON `post`.`traderID`=`trader`.`traderID`
           left JOIN `productbike` ON (`post`.`productCategoryID`=`productbike`.`productCategoryID` AND `post`.`productID`=`productbike`.`productID`)
           where productbike.productCategoryID=2 and (productbike.productBBrand like '%" . $carBrand . "' and productbike.productBmodel like '%" . $carModel . "' and productbike.productBReleaseYear like '%" . $carStartDate . "' and productbike.productBReleaseYear like '%" . $carEndDate . "') limit " . $per_page_cnt . " offset " . $limit . " ");
            return $qry->result();
...
productcar.productCPrice ,productcar.Cpost_main_img ,concat(productcar.productCBrand,productcar.productCModel) as product_name1,productcar.cartCType, 
    ...
    function recent_view() {
        $recentqry = $this->db->query('SELECT * FROM `productiv` order by productViewCount desc LIMIT 4');
...
//        $this->db->limit($limit, $offset);
    ...
    function get_cart_traderid($userId) {
        ...
        left JOIN `trader` ON `cartlist`.`userID`=`trader`.`traderID`
    ...
    function get_favourite_traderid($userId) {
        $qry = $this->db->query('SELECT watchlist.watchlistID,watchlist.postID, p.productCategoryID,watchlist.productID,
         ...
         from watchlist
         left JOIN `trader` ON `watchlist`.`userID`=`trader`.`traderID`
...
//         echo $query;exit();
  ...
  function get_post_details($postid){
    ...
    function new_registers(){
        $new_reg_count = $this->db->query('select * from vwTrader where planStatus=0');
    ...
    function total_watchlist(){
      $query = $this->db->query('select SUM(watchlistCount) as count from watchlist');
      $query =  $query->row();
      return $query->count;
     }
     ...
     function trader_paymentdetails($trader_id) {
        $this->db->select('plan.planID,planStatus,paymentTypeChosen,plan.planName,tradersubscriptionplan.planPostCount,tradersubscriptionplan.planValidity,');
        $this->db->from('tradersubscriptionplan');
        $this->db->where('traderID', $trader_id);
        $this->db->join('plan', 'plan.planID = tradersubscriptionplan.planID');
        $query = $this->db->get();
         //echo $this->db->last_query();
        return $query->result();
    }

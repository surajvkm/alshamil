        ...
        <div class="col-lg-12">
            <div class="row">
                ...
                <div class="col-main">
                    ...
                    <form id="myForm" enctype="multipart/form-data" method="post" action="<?php echo base_url();?>/admin/updatepost/<?php echo $product_id ?>">
<input type="hidden" value="<?php echo $rc->productCategoryId ?>" name="txtcat" />
                    <div class="row">
                                <div class="col-lg-12">
                                    <form action="">
                                        <div class="row">
                                                ...
                                                <!-- --- Category --- -->
                                                    ...
                                                    <div class="card-inp pl-0" >
                                                        <div class="form-group" >
                                                            <label class="text-s12 text-semibold" for="">Category
                                                            
                                                           
                                                            </label>
                                                      
                                                            <select disabled=""  class="form-control input-custom" id="txtcategorya" name="txtcat">
                                                                ...
                                                                foreach ($cat_qry->result() as $r) {
                                                                    ...
                                                                    ?>
                <option  <?php if($rc->productCategoryId===$r->CategoryId) echo 'selected'  ?> value="<?php echo $r->CategoryId ?>"><?php echo $r->Name ?></option>
                                                                    <?php
                                                                }
                                                                ?>
...
$qry    = $this->adm->rcv_categories($rc->productCategoryId);
...
$this->db->where('parentCategory',$rc->productCategoryId);
...
?>
<option value="<?php echo $row->id ?>"  <?php if($rc->subCategory1Id==$row->id) echo 'selected' ?>   ><?php echo $row->name ?></option>
<?php  endforeach; } }
elseif($key==1){ 
$this->db->select('CategoryId as id, Name as name');
$this->db->where('CategoryId',$rc->subCategory2Id);
$this->db->order_by('parentCategory','asc');
$records	= $this->db->get_where('category');	
...
<?php endforeach; } else{ 
$this->db->select('CategoryId as id, Name as name');
$this->db->where('CategoryId',$rc->subCategory3Id);
$this->db->order_by('parentCategory','asc');
$records	= $this->db->get_where('category');	
foreach($records->result() as $row2):
?>
...
<?php } elseif($type==2) {
	...
	$jsonbrand = json_decode($rc->brand);
	$template_row=  $this->adm->get_templatesById($jsonbrand->templateId);
	$template_qry = $this->adm->get_templatesByType($template_row->type);
	$tempcode  =   $this->adm->get_template_code($jsonbrand->templateId);
	$productmedia =$this->db->select('productImage')->where('productId',$product_id)->get('productmedia')->row();
...
<div class="col-6">
    <div class="form-group"> <label for="usr" class="text-s12 text-semibold">Vehicle</label>
        <select class="form-control input-custom" id="vehicle_typep" name="vehicle_type">
           ...
           <option value="0" <?php if($template_row->type==0) echo 'selected' ?>   >Car</option>
           <option value="1"  <?php if($template_row->type==1) echo 'selected' ?> >Bike</option>
        </select>
    ...
    <div class="form-group">
        ...
        <select class="form-control  input-custom fontsize-12" id="txtemirates" style="width: 177px;" name="txtemirates">
            ...
            foreach ($template_qry as $r) {
                ?>
                <option value="<?php echo $r->noplateTempID ?>"   <?php if($template_row->noplateTempID==$r->noplateTempID) echo 'selected' ?>    ><?php echo $r->emirates ?></option>
                <?php
            }
            ?>
 ...
 <div id="codeDiv" class="col-6" >
        <div class="form-group">
            ...
            <select class="form-control  input-custom" name="txtcode" id="txtcode">
               <?php foreach ($tempcode as $row) {  ?>
               <option value="<?php echo $row ?>"  <?php if($row==$jsonbrand->txtcode) echo 'selected' ?> ><?php echo $row ?></option>
               <?php } ?>
    ...
    <div class="col-lg-5 col-md-5 col-5 ml-lg-3 ml-md-3 ml-3" id="template_coldiv" >
        <div class="form-group">
            <div class="row show-plate">
                ...
                <div id="short_tmplt">
                <img src="<?php echo $rc->mainImage ?>" width="130"/>
                </div>
                <img style="display: none" src="<?php echo $template_row->templates ?>" id="template_img" style="width:130px;">
       ...
       <div class="col-lg-5 col-md-5 col-5 ml-lg-5 ml-md-5 ml-4 ml-sm-5" id="template_coldiv" >
        <div class="form-group">
            <div class="row show-plate pl-lg-3">
                ...
                <div id="loading1" style="display: none"><i class="fa fa-cog fa-spin fa-2x fa-fw"></i> Loading..</div>
                <div id="long_tmplt">
                	<img src="<?php if(count($productmedia)) echo $productmedia->productImage ?>" width="130"/>
                </div>
                <img style="display: none" src="<?php echo $template_row->long_template ?>" id="long_template_img" style="width:130px;">
...
<div class="form-group ">
        ...
        <select class="form-control input-custom" name="txtno_digs" id="txtno_digs">
            <option value="">--Select--</option>
            <option value="1"  <?php if(1==$jsonbrand->txtno_digs) echo 'selected' ?> >1</option>
            <option value="2"  <?php if(2==$jsonbrand->txtno_digs) echo 'selected' ?> >2</option>
            <option value="3"  <?php if(3==$jsonbrand->txtno_digs) echo 'selected' ?> >3</option>
            <option value="4"  <?php if(4==$jsonbrand->txtno_digs) echo 'selected' ?> >4</option>
            <option value="5"  <?php if(5==$jsonbrand->txtno_digs) echo 'selected' ?> >5</option>
        </select>
</div>
</div>
<!-- --- Add price --- -->
<div class="col-6">
  <div class="form-group">
            ...
            <input type="text" value="<?php echo $jsonbrand->txtnumber  ?>" name="txtnumber" id="txtno" class="form-control input-custom">
...
<div class="col-6"   >
    <div class="form-group">
        ...
        <input type="hidden" name="operator" id="operator" value="<?php echo $jdata->operator; ?>"/>
        ...
        <div class="form-group">
            <div class="show-plate mobile-plate">
                ...
                <img style="display: none" src="" id="template_img" style="width:130px;">
                 ...
                 <?php
                                                if ($rc->mainImage != '' && (@getimagesize($rc->mainImage))) {
                                                ?>
                                                    <img src="<?php echo $rc->mainImage ?>" id="newImg" class="latest_postimgs lazy" width="130"/>
                                                <?php
                                                } else {
                                                ?>
                                                    <img src="<?php echo base_url() ?>img/no_preview.png" id="newImg" class="latest_postimgs lazy" width="130"/>
                                                <?php
                                                }
                                                ?>
...
<div class="col-6">
   <div class="form-group">
        ...
        <input type="text" pattern=".{5,13}" required value="<?php echo $jdata->txtmob ?>" title="Enter 5 to 13 characters" name="txtmob" class="form-control input-custom" id="txtmobb" required >
...
<input type="text" name="txt_prtitle"  id="txt_prtitle" value="<?php echo $rc->productTitle ?>" class="form-control input-custom" >
...
<input type="text" name="txt_prtitle_ar" value="<?php echo $rc->productTitleAr ?>"  id="txt_prtitle_ar" class="form-control input-custom" >
...
<div class="custom-control custom-checkbox ml-lg-3">
    ...
    <input type="checkbox"  class="custom-control-input" <?php if($rc->callForPrice) echo 'checked' ?>  name="call_for_price" id="chkbox_callpr1">
...
<input type="text" name="txtprice"  id="txt_cprice" value="<?php if($rc->price) echo $rc->price ?>" <?php if($rc->callForPrice) echo 'disabled' ?> class="form-control input-custom" >
...
<textarea id="txtdetails" name="txtdetails" class="form-control input-custom" id="" cols="30" rows="5"><?php echo $rc->description ?></textarea>
...
<textarea id="txtdetails_ar" name="txtdetails_ar" class="form-control input-custom" id="" cols="30" rows="5"><?php echo $rc->descriptionAr ?></textarea>
    ...
    <div class="card add-card">
        ...
        <div class="addIcon-P h-100">
            ...
            <div class="dropZoneOverlay">
                    ...
                    <img id="audi_prev_icon"  alt=''  src='<?php echo $rc->mainImage ?>'>
                </div>
                    </div>
        </div>
  ...
  if (file.type.match('image')) {
    ...
    fileReader.readAsArrayBuffer(file);
                                                    ...
                                                    <div class="col-lg-9 mx-auto">
                                                        <div class="row">
                                                           ...
                                                           <?php
                                                            ...
                                                            if($rc->postStatusDetail!=1){?>
                                                            <?php if($rc->type!=3){?>
                                                            <div class="col-4">
                                                                <button class="btn btn-success text-s15 textresize w-100 pt-2 pb-2 btn_approve" <?php if($rc->postStatusDetail==1)echo "disabled"; ?> value="<?php echo $rc->productId?>" id='btn_newpost_appr'>
                                                                    APPROVE
                                                                </button>
                                                            ...
                                                            <div class="col-4">
                                                              ...
                                                              <button type='submit' id="btn_newpost_update" class="btn btn-orange text-s15 textresize w-100 pt-2 pb-2 btn-update" <?php //if($r->PostAdminStatus==1)echo "disabled"; ?> value="<?php echo $rc->productId?>">UPDATE</button>
                                                            ...
                                                            <?php if($rc->type!=3){?>
                                                            <div class="col-4">
                                                            <button id="btn_newpost_rej" class="btn btn-red text-s15 textresize w-100 pt-2 pb-2 btn-reject" <?php if($rc->postStatusDetail=='-1')echo "disabled"; ?> value="<?php echo $rc->productId?>">REJECT</button>
                               ...
                               <!-- --- Buttons --- -->
                                                ...
                                                <div class="row mt-4" id='msgbox' <?php if($rc->postStatusDetail!='-1'){?> style="display: none;" <?php } ?>
                                                    <div class="col-12">
                                                    <textarea id="msg" cols="30" rows="3" name="txtemail" id="postDet_Message"  class="form-control input-custom "><?php if(!empty($rc->rejectMsg)) echo $rc->rejectMsg;?></textarea>
                                                    <div class="col-lg-3 col-md-4 col-sm-4 col-6 ml-auto mt-2">
                                                        <button id="postDet_BtnSend" value="<?php echo $rc->productId?>"  class="btn btn-orange text-s14 w-100 pt-2 pb-2 text-normal">
                                                            Send Message
                                                        </button>
                                                    </div>
                                                    </div>
